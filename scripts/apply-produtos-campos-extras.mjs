...
const migrationPath = resolve(
  process.cwd(),
  "supabase/migrations/20260617200000_produtos_campos_extras.sql",
);
...
async function columnExists() {
  ...
  const { error } = await admin.from("produtos").select("codigo_barras").limit(1);
  return !error;
}
...
async function main() {
  ...
  if (await columnExists()) {
    console.log("OK: colunas extras de produtos ja existem no Supabase.");
    return;
  }
  ...
  if (await columnExists()) {
    console.log("Migration produtos_campos_extras aplicada com sucesso.");
    return;
  }
