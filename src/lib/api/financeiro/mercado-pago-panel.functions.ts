...
export const fetchMercadoPagoPanelServer = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }): Promise<MercadoPagoPanelSnapshot> => {
    ...
    const { getIntegrationStatus } = await import("@/lib/api/tenant/integrations.functions");
    ...
    try {
      const { mercadoPagoSearchPayments } = await import("@/lib/api/financeiro/mercado-pago.server");
      const search = await mercadoPagoSearchPayments({ limit: 30, days: 30 });

      let availableBalance: number | null = null;
      try {
        const { mercadoPagoGetBalance } = await import("@/lib/api/financeiro/mercado-pago.server");
