...
} from "@/lib/api/fiscal-store.server";

async function emitNfceViaSefaz(
  ...args: Parameters<(typeof import("@/lib/api/fiscal-sefaz.server"))["emitNfceViaSefaz"]>
) {
  const { emitNfceViaSefaz: emit } = await import("@/lib/api/fiscal-sefaz.server");
  return emit(...args);
}

async function checkSefazStatus(secrets: SefazSecrets) {
  const { checkSefazStatus: check } = await import("@/lib/api/fiscal-sefaz.server");
...
export async function consultarStatusNotaFiscal(notaId: string) {
  ...
  const secrets = await loadSefazSecrets(settings.config.ambiente);
  const { consultarProtocoloSefaz } = await import("@/lib/api/fiscal-sefaz.server");
...
export async function cancelarNotaFiscal(notaId: string, justificativa: string) {
  ...
  const secrets = await loadSefazSecrets(settings.config.ambiente);
  const { cancelarNfceSefaz } = await import("@/lib/api/fiscal-sefaz.server");
...
export async function inutilizarNumeracaoFiscal(input: {
  ...
  const secrets = await loadSefazSecrets(settings.config.ambiente);
  const { inutilizarNumeracaoSefaz } = await import("@/lib/api/fiscal-sefaz.server");
