...
import { getInstanceOwner, pickChatDisplayName } from "@/lib/api/whatsapp-identity.server";
...
export async function consolidateOrphanLidChats(
  ...
  const schemaReady = await isWhatsAppSchemaReady();
  const { getInstanceOwner, isOwnerJid } = await import("@/lib/api/whatsapp-identity.server");
  const owner = await getInstanceOwner();
...
export async function resolvePhoneFromChatMessages(chatId: string) {
  const { toEvolutionSendDigits } = await import("@/lib/whatsapp");
  const schemaReady = await isWhatsAppSchemaReady();
  if (!schemaReady) {
    for (const message of getMemoryState().messages) {
...
export { extractPhoneJidFromMessageKey, pickMessageRemoteJid } from "@/lib/whatsapp";
