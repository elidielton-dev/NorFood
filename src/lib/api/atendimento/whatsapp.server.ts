...
import {
  ...
  upsertWhatsAppChat,
} from "@/lib/api/atendimento/whatsapp-store.server";
import type {
  WhatsAppInboxState,
  WhatsAppListMode,
  WhatsAppMessageType,
...
} from "@/lib/api/atendimento/whatsapp-identity.server";
...
async function resolveWebhookChatMirror(input: {
  ...
  if (!existingChat && input.chatRemoteJid.endsWith("@lid")) {
    const { findExistingChatForIncomingLid } = await import("@/lib/api/atendimento/whatsapp-store.server");
...
async function ensureWhatsAppMessageMediaStored(input: {
  ...
  const { updateWhatsAppMessageMediaUrl } = await import("@/lib/api/atendimento/whatsapp-store.server");
  const { fetchEvolutionMediaBase64 } = await import("@/lib/api/atendimento/whatsapp-baileys.server");
...
export async function processIncomingWhatsAppRecord(record: Record<string, unknown>) {
  ...
  if (isInbound && savePhone) {
    ...
    if (contact?.id) {
      const { countWhatsAppInboundMessages } = await import("@/lib/api/atendimento/whatsapp-store.server");
...
export async function resolveAndPersistChatContact(input: {
  ...
  if (!phone && input.chatRemoteJid.endsWith("@lid")) {
    ...
    if (!phone) {
      const { pullRealPhoneForLidChat } = await import("@/lib/api/atendimento/whatsapp-identity.server");
...
export async function refreshWhatsAppChatProfilePicture(
  ...
  const { getWhatsAppChatById, updateChatIdentityInPlace } =
    await import("@/lib/api/atendimento/whatsapp-store.server");
  const { fetchEvolutionProfilePicture } = await import("@/lib/api/atendimento/whatsapp-baileys.server");
...
async function ensureChatReadyForSend(chat: {
  ...
  const { promoteChatToRealPhoneJid } = await import("@/lib/api/atendimento/whatsapp-store.server");
...
export async function startWhatsAppConnection() {
  ...
  const { resetEvolutionInstanceSession } = await import("@/lib/api/atendimento/whatsapp-baileys.server");
...
async function syncRecentMessagesForChatMirror(
  ...
  const { touchWhatsAppChatPreview } = await import("@/lib/api/atendimento/whatsapp-store.server");
...
export async function sendWhatsAppTextMessage(
  ...
  let quoted: import("@/lib/api/atendimento/whatsapp-baileys.server").EvolutionQuotedMessage | undefined;
  let replyToWaMessageId: string | null = null;
  let replyToText: string | null = null;
  let replyToFromMe: boolean | null = null;

  if (options?.quotedMessageId) {
    const { getWhatsAppMessageRow } = await import("@/lib/api/atendimento/whatsapp-store.server");
    ...
    if (quotedRow) {
      const quotedMessage = {
        ...
        body: quotedRow.body,
        messageType: quotedRow.message_type as import("@/lib/atendimento/whatsapp").WhatsAppMessageType,
      ...
      replyToText =
        ...
        (quotedRow.message_type === "document" && quotedRow.file_name?.trim()
          ...
          : null) ||
        mediaTypeLabel(quotedRow.message_type as import("@/lib/atendimento/whatsapp").WhatsAppMessageType);
...
export async function processWhatsAppMessageStatusUpdate(record: {
  ...
  const { updateWhatsAppMessageStatusByWaId } = await import("@/lib/api/atendimento/whatsapp-store.server");
...
export async function getWhatsAppSetupInfo() {
  const { getEvolutionPublicConfig } = await import("@/lib/api/atendimento/whatsapp-baileys.server");
  ...
  return {
    ...
    webhookUrl: (await import("@/lib/api/atendimento/whatsapp-baileys.server")).getPublicWebhookUrl(),
