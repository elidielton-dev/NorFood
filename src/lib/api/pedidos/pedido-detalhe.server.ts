import { getOrderMetadataValue, getOrderNeighborhood } from "@/lib/shared/db";
import {
  getOrderAddressForDisplay,
  getOrderFreeNotes,
  getOrderPhoneForDisplay,
  getOrderReferenceForDisplay,
} from "@/lib/delivery/order-display";
import type { VendaDetalhe, VendaDetalheItem, VendaDetalheNota } from "@/lib/pedidos/venda-detalhe";
import { labelCanal } from "@/lib/relatorios/relatorios-inteligencia";
...
export async function fetchVendaDetalhe(pedidoId: string, tenantId: string): Promise<VendaDetalhe> {
  ...
  if (!tenantId) throw new Error("Tenant obrigatório para detalhe do pedido.");

  const pedidoQuery = supabaseAdmin
    .from("pedidos")
    .select("*")
    .eq("id", pedidoId)
