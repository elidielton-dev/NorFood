...
export const SERVICE_CITY_CONFIG = {
  ...
  ] satisfies SupportedNeighborhood[],
} as const;

export function isSupportedCityCep(value: string) {
  return value.replace(/\D/g, "") === SERVICE_CITY_CONFIG.cep.replace(/\D/g, "");
}

export function getSupportedNeighborhoods() {
  return [...SERVICE_CITY_CONFIG.neighborhoods];
}

export function findSupportedNeighborhood(value: string) {
  const normalized = normalizeComparableText(value);
  return (
    SERVICE_CITY_CONFIG.neighborhoods.find((item) => {
      const aliases: string[] =
        "aliases" in item && Array.isArray(item.aliases) ? item.aliases : [];
      const candidates = [item.name, ...aliases].map(normalizeComparableText);
      return candidates.some(
        (candidate) => comparableFormatter.compare(candidate, normalized) === 0,
      );
    }) ?? null
  );
}

export function getNeighborhoodDeliveryFee(value: string) {
  return findSupportedNeighborhood(value)?.deliveryFee ?? SERVICE_CITY_CONFIG.defaultDeliveryFee;
}

export function getNeighborhoodCoordinates(value: string) {
  const neighborhood = findSupportedNeighborhood(value);
...
export function getCityLabel() {
  return `${SERVICE_CITY_CONFIG.city}/${SERVICE_CITY_CONFIG.state}`;
}
