...
const SITE_NOTICE_STORAGE_KEY = "norfood-site-notice";

/** Inline (sem arquivo separado) para evitar bloqueio por adblock em paths com consent/cookie. */
function LandingSiteNotice() {
  const [open, setOpen] = useState(false);

  useEffect(() => {
    try {
      const accepted = localStorage.getItem(SITE_NOTICE_STORAGE_KEY);
  ...
  function accept() {
    try {
      localStorage.setItem(SITE_NOTICE_STORAGE_KEY, "1");
    } catch {
      /* ignore */
    }
    setOpen(false);
      ...
      <div className="mx-auto flex max-w-6xl flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex gap-3">
          ...
          <div className="text-sm text-[#5C4A3A]">
            <p className="font-semibold text-[#1A1A1A]">Privacidade e experiência</p>
            <p className="mt-0.5 leading-relaxed">
              Usamos dados locais para melhorar sua navegação. Ao continuar, você concorda com nossa
