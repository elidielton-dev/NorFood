...
import { getDeliveryFeeForNeighborhood } from "@/lib/delivery/delivery-pricing";
...
import { OrderLiveMap } from "@/components/delivery/order-live-map-lazy";
import { createDeliveryOrder } from "@/lib/api/pedidos/orders.functions";
import { getTenantAccessStatusServer } from "@/lib/api/financeiro/platform-billing.functions";
import { createMesaQrOrder, resolveMesaByToken } from "@/lib/api/pedidos/mesa-order.functions";
import { getMesaGuestName, setMesaGuestName } from "@/lib/mesa-guest";
import { validateCouponServer } from "@/lib/api/tenant/coupons.functions";
import { expirePendingMercadoPagoOrders } from "@/lib/api/financeiro/mercado-pago.functions";
import {
  requestBrowserDeviceLocation,
  type BrowserDeviceLocation,
} from "@/lib/shared/browser-geolocation";
    ...
    <div className={`rounded-3xl border border-border bg-card p-5 shadow-soft ${className}`.trim()}>
      ...
      {isOpen && orderId ? (
        <div className="mt-4 overflow-hidden rounded-[2rem]">
          <OrderLiveMap orderId={orderId} orderNumber={orderNumber} />
        </div>
      ) : null}
    </div>
  );
...
function OrderDetailPanel({ detail }: { detail: CustomerOrderDetail }) {
  ...
  return (
    <div className="mt-4 rounded-2xl border border-border bg-card p-4">
      ...
      {showTrackingMap ? (
        <div className="mb-4">
          <OrderLiveMap orderId={detail.id} orderNumber={detail.numero} />
        </div>
      ) : null}
