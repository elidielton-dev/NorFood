...
function DeliveryFlowPage() {
  ...
  useEffect(() => {
    ...
    if (temPedidoNovo) {
      tocarAlerta();
      toast.success("Novo pedido entrou no KDS");
    }
  ...
  useEffect(() => {
    ...
    const alvo = document.getElementById("kds-separacao");
    ...
    <div className="kds-board -mx-4 min-h-[calc(100vh-6rem)] bg-panel-muted font-sans sm:-mx-6 lg:-mx-8">
      ...
      >
        {kanbanColumns.map((column) => (
          <div
            key={column.key}
            id={column.key === "separacao" ? "kds-separacao" : undefined}
            className={cn(
              foco === "separacao" &&
